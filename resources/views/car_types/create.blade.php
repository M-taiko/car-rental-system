...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">إضافة نوع سيارة جديد</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('car-types.store') }}" id="carTypeForm">
                        @csrf
                                        ...
                                        <tbody>
                                            @foreach($workShifts as $shift)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="hidden" name="shift_rates[{{ $loop->index }}][work_shift_id]" value="{{ $shift->id }}">
                                                        <input type="number" 
                                                               step="0.01" 
                                                               min="0" 
                                                               class="form-control @error('shift_rates.' . $loop->index . '.rate') is-invalid @enderror" 
                                                               name="shift_rates[{{ $loop->index }}][rate]" 
                                                               value="{{ old('shift_rates.' . $loop->index . '.rate', 0) }}" 
                                                               required>
                                                        @error('shift_rates.' . $loop->index . '.rate')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
                                                        @enderror
